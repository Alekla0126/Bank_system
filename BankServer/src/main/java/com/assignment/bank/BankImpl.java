...
public class BankImpl extends UnicastRemoteObject implements BankInterface
{
    ...
    @Override
    /**
     * @desc checks if the account number given by the user exists in the
     * database
    ...
    @Override
    /**
     * @desc deposits the amount entered by the user into the account number and
    ...
    @Override
    /**
     * @desc withdraws the amount entered by the user from the bank account and
     * saves the transaction details
     * @param int acnt - the user account number
    ...
    @Override
    /**
     * @desc gets transactions from a start date to end date from the database
     * and send an email with the list of transactions
     * @param int acnt - the account number
     * @param int String startDate - the transaction start date
     * @param int String endDate - the transaction end date
     * @param int String emailAddr - the email address for sending the
     * transactions to
     * @return void
     */
    public void getTransactions(int acnt, String startDate, String endDate, String emailAddr) throws RemoteException
    {
        ...
        String fromUser = "yaballacu@gmail.com";
        String fromUserEmailPassword = "chispaS14-/";
        try
        {
    ...
    @Override
    /**
     * @desc makes payment to a vendor and saves the transaction details to the
     * database
     * @param int acnt - the user account number
    ...
    public String makePayment(int acnt, int amt, int payAccNumber, String payVendor) throws RemoteException
    {
        ...
        try
        {
            ...
            trans.setTransDetails("Pago de $" + amt + " hecho a " + payVendor + " con el número de cuenta " + payAccNumber);
            session.save(trans);
            t3.commit();
        }
        catch (HibernateException e)
